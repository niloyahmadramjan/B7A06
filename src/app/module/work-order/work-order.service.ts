...
const updateWorkOrder = async (
  ...
  if (
    ...
    !(payload.technicianId ?? existing.technicianId)
  ) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "A technician is required before assigning a work order",
    );
  }
  if (
    ...
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "A technician is required before starting a work order",
    );
  }
...
const deleteWorkOrder = async (workOrderId: string) => {
  ...
  if (!existing)
    throw new AppError(httpStatus.NOT_FOUND, "Work order not found");
  if (
    existing.status === WorkOrderStatus.IN_PROGRESS ||
    existing.status === WorkOrderStatus.COMPLETED
  ) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "In-progress or completed work orders cannot be deleted",
    );
  }
